...
`skill-trees.html` embeds the full skill catalog as a large inline JS object (`SKILLS_PART1`–`SKILLS_PART5`) — nothing flags mismatches automatically. A generator script may exist in `Tools/` under the Coming Soon folder (not confirmed; requires that folder to be connected).

**Options:** full resync pass once Coming Soon is connected, or write a generator that builds these from the same `.md` sources `build.js` already reads.

---

## Website — Post-MVP d20 Conversion Tools

> **Deferred.** Build after the first GitHub Pages publish.
