...
Not every action calls for dice. Per Velocity's drama-over-detail principle (A Core Design Philosophy), the Narrator only calls for an Opposition Check when at least one of these is true:

- The outcome is genuinely uncertain — a reasonable Narrator wouldn't already know how it goes.
...
An Opposition Check follows three steps:

1. Both sides roll their dice pool. Every character rolls a minimum of 1d10 regardless of skills. Skills that explicitly grant dice add to this base. Flat bonuses from applicable skills are added after rolling. Each side then adds their relevant Attribute as a flat bonus.
2. Each side totals their result. The side with the highest total wins the contest.
3. If both sides arrive at the same total, the result is a stalemate.
...
A character who is fully absorbed in an activity demanding total focus becomes Vulnerable to physical attacks. This applies when a character is:

- engaged in a sustained supernatural contest (such as a Willpower vs. Willpower spell duel or domination attempt)  
- maintaining a complex ritual or channeling ability that requires uninterrupted focus  
- performing a delicate or high‑risk technical action where a single distraction would cause failure  
- using a skill or ability that explicitly states it imposes Vulnerable  

A Vulnerable character:

- cannot roll Reflex to oppose melee or ranged attacks  
...
If both sides arrive at the same total, the contest results in a stalemate.

A stalemate represents:

- a deadlock in a grapple  
- a moment of hesitation in a negotiation  
- a spell duel where neither caster gains ground  
- a physical attack narrowly avoided or disrupted  
- a supernatural clash with no clear victor  

The Narrator determines whether the stalemate consumes time, resources, or positioning, depending on the fiction. When a stalemate occurs, the Narrator should turn to the player and ask: "Neither of you gives ground. What do you do?" The stalemate is not the end of the scene — it is a moment of decision.

## Armor and Opposition Checks

Armor does not influence Opposition Checks. It does not prevent hits or make characters harder to strike. Instead, armor reduces incoming damage after a successful attack.

This separation ensures that:

- Reflex determines whether a character avoids harm  
- Armor determines how much harm they suffer  
- Attributes remain the primary drivers of success  
- Armor remains a meaningful but secondary layer of defense  

## Narrator Guidance

- Use the Attribute pairings consistently to reinforce the logic of the world.  
- Let stalemates create tension rather than frustration.  
- Apply the Vulnerable condition whenever a character is fully committed to a supernatural contest.  
- Keep the focus on narrative consequences: every Opposition Check should matter.  
- Remember that armor mitigates damage, not hit chance.  
- Encourage players to exploit openings, flank distracted enemies, and protect allies engaged in supernatural duels.
