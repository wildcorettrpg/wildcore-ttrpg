# Weapons

## Purpose

1. This table defines the base damage die for every weapon category referenced by Melee Attack and Ranged Attack.
2. A weapon's base damage die is the die rolled before any attribute or skill damage bonuses are applied (see [Damage](../Encounters/Damage.md) §2-3).
3. Bonus damage dice from attributes and skills use this same die (per [Damage](../Encounters/Damage.md) §3.4).
4. Each weapon also lists a **Size** (Tiny/Small/Medium/Large, per [Inventory](Inventory.md) §2) for inventory purposes and a **Cost** in gold pieces (gp) for purchase. Costs are ported from d20 source material — see [Currency](Currency.md) for the currency note. Cost has no effect on inventory slots or carrying capacity.

## Melee Weapons

| Item         | Category     | Base Damage | Properties        | Size   |  Cost |
| :----------- | :----------- | :---------- | :---------------- | :----- | ----: |
| Battleaxe    | Medium melee | 1d10        | —                 | Medium | 10 gp |
| Club         | Light melee  | 1d6         | —                 | Small  |     — |
| Dagger       | Light melee  | 1d6         | —                 | Small  |  2 gp |
| Flail        | Medium melee | 1d10        | —                 | Medium | 10 gp |
| Glaive       | Heavy melee  | 1d12        | Reach, Two-handed | Large  | 20 gp |
| Greataxe     | Heavy melee  | 3d6         | Two-handed        | Large  | 30 gp |
| Greatclub    | Medium melee | 1d10        | Two-handed        | Large  |  2 sp |
| Greatsword   | Heavy melee  | 3d6         | Two-handed        | Large  | 50 gp |
| Halberd      | Heavy melee  | 1d12        | Reach, Two-handed | Large  | 10 gp |
| Handaxe      | Light melee  | 1d8         | —                 | Small  |  5 gp |
| Knife        | Light melee  | 1d6         | —                 | Small  |  1 gp |
| Lance        | Heavy melee  | 3d6         | Reach, Two-handed | Large  | 10 gp |
| Light hammer | Light melee  | 1d6         | —                 | Small  |  2 gp |
| Longsword    | Medium melee | 1d10        | —                 | Medium | 15 gp |
| Mace         | Medium melee | 1d8         | —                 | Medium |  5 gp |
| Maul         | Heavy melee  | 3d6         | Two-handed        | Large  | 12 gp |
| Morningstar  | Medium melee | 1d10        | —                 | Medium | 15 gp |
| Pike         | Heavy melee  | 1d12        | Reach, Two-handed | Large  |  5 gp |
| Quarterstaff | Medium melee | 1d8         | Two-handed        | Medium |  2 sp |
| Rapier       | Medium melee | 1d10        | —                 | Medium | 25 gp |
| Scimitar     | Light melee  | 1d8         | —                 | Small  | 25 gp |
| Shortsword   | Light melee  | 1d8         | —                 | Small  | 10 gp |
| Sickle       | Light melee  | 1d6         | —                 | Small  |  1 gp |
| Spear        | Medium melee | 1d8         | Two-handed        | Medium |  2 gp |
| Trident      | Medium melee | 1d8         | —                 | Medium |  5 gp |
| War pick     | Medium melee | 1d10        | —                 | Medium |  5 gp |
| Warhammer    | Medium melee | 1d10        | —                 | Medium | 15 gp |
| Whip         | Light melee  | 1d6         | Reach             | Small  |  2 gp |

1. All melee weapons have a default reach of 5 ft (one square). Weapons with the **Reach** property extend this to 10 ft (two squares) and can target opponents in the adjacent square beyond normal melee range.

## Ranged Weapons

| Item | Category | Base Damage | Range | Size | Cost |
| :--- | :-------- | :---------- | :---- | :--- | ----: |
| Dart | Light ranged | 1d6 | 20 / 60 ft | Tiny | 5 cp |
| Hand crossbow | Light ranged | 1d8 | 30 / 120 ft | Small | 100 gp |
| Heavy crossbow | Heavy ranged | 1d12 | 100 / 400 ft | Large | 50 gp |
| Javelin | Medium ranged | 1d8 | 30 / 120 ft | Medium | 1 gp |
| Light crossbow | Medium ranged | 1d10 | 80 / 320 ft | Medium | 35 gp |
| Longbow | Heavy ranged | 1d10 | 150 / 600 ft | Large | 50 gp |
| Shortbow | Medium ranged | 1d8 | 80 / 320 ft | Medium | 25 gp |
| Sling | Light ranged | 1d6 | 30 / 120 ft | Tiny | — |
| Thrown handaxe | Light ranged | 1d8 | 20 / 60 ft | Small | 5 gp |
| Thrown knife | Light ranged | 1d6 | 20 / 60 ft | Small | 1 gp |
| Thrown light hammer | Light ranged | 1d6 | 20 / 60 ft | Small | 2 gp |
| Thrown spear | Medium ranged | 1d8 | 20 / 60 ft | Medium | 2 gp |
| Thrown trident | Medium ranged | 1d8 | 20 / 60 ft | Medium | 5 gp |

1. Range is listed as **normal / max** in feet, using the same 5 ft grid as movement. Attacks within normal range have no penalty. Attacks beyond normal range up to max range take a −2 to the attack Opposition Check. Attacks beyond max range are not possible.
2. Range values are ported from d20 5e source material.

## Unarmed

1. Unarmed attacks have no base damage die — damage is Strength bonus only (minimum 1).
2. Unarmed damage equals the attacker's flat Strength bonus, minimum 1 point.
3. Unarmed attacks have no Size or Cost — they require no equipment.

## Ammunition

1. Ranged weapons that require ammunition (bows, crossbows, slings) consume ammunition appropriate to the weapon. See the tables below for types and costs.
2. Each bundle of 20 pieces of ammunition is Medium and occupies one inventory slot. Bundles do not stack.
3. Thrown weapons (dart, javelin, thrown handaxe, thrown knife, thrown light hammer, thrown spear, thrown trident) are their own ammunition — each throw uses the weapon itself.

### Standard Ammunition

| Item | Size | Cost | Compatible Weapons |
| :--- | :--- | ---: | :--- |
| Arrows (20) | Medium | 1 gp | Shortbow, longbow |
| Bolts (20) | Medium | 1 gp | Hand crossbow, light crossbow, heavy crossbow |
| Sling bullets (20) | Medium | 2 sp | Sling |
| Sling stones (20) | Medium | — | Sling; improvised — −2 to attack |

### Special Ammunition

Special ammunition replaces standard ammunition for that attack and is consumed on use. Arrows pair with bows; bolts pair with crossbows.

| Item | Size | Cost | Effect |
| :--- | :--- | ---: | :--- |
| Arrows, blunt (20) | Medium | 2 gp | On a hit, the target becomes [Hindered](../Conditions%20and%20Hazards/Conditions.md#hindered) until the end of their next turn. Deals no damage. |
| Arrows, broadhead (20) | Medium | 3 gp | +1d6 damage on a hit. |
| Arrows, flight (20) | Medium | 2 gp | Eliminates the long-range −2 penalty. You may shoot up to the weapon's max range without penalty. |
| Arrows, serrated (20) | Medium | 3 gp | On a hit, the target gains the [Bleeding Out](../Conditions%20and%20Hazards/Conditions.md#bleeding-out) condition. |
| Arrows, whistling (20) | Medium | 2 gp | Audible to 300 ft. Used for signals; no combat effect. |
| Bolts, blunt (20) | Medium | 2 gp | On a hit, the target becomes [Hindered](../Conditions%20and%20Hazards/Conditions.md#hindered) until the end of their next turn. Deals no damage. |
| Bolts, broadhead (20) | Medium | 3 gp | +1d6 damage on a hit. |
| Bolts, serrated (20) | Medium | 3 gp | On a hit, the target gains the [Bleeding Out](../Conditions%20and%20Hazards/Conditions.md#bleeding-out) condition. |

## Special Materials

Certain weapon materials deal 1 additional damage die against specific races and lineages. The additional die uses the same die type as the weapon's base damage die.

| Material | Additional Die vs. | Notes |
| :--- | :--- | :--- |
| Iron or steel | [Faerie](../Races/Faerie%20Race.md) race; [Fae-Touched](../Skills/Lineages/Fae-Touched%20Lineage.md) lineage | Standard material for most metal weapons. No surcharge. |
| Silver | [Vampire](../Skills/Lineages/Vampire%20Lineage.md) lineage; [Werewolf](../Skills/Lineages/Werewolf%20Lineage.md) lineage | Special order. ×10 base weapon cost. Silver arrows and bolts are available at 10 gp per 20. |

1. Most metal weapons are iron or steel by default. Weapons made of other materials (wooden clubs, bone knives, obsidian blades) do not trigger the iron/steel bonus.
2. Silver weapons are softer than steel â they are not inherently better against other targets, just specifically effective against Vampire and Werewolf lineages.
3. Dual-material weapons (e.g., a silver blade on an iron crossguard) are not supported. A weapon is either silver or iron/steel, not both.

## Weapon Coatings

Silver and wolfsbane can be applied as coatings to bladed weapons and arrowheads to deliver their effects against supernatural creatures without forging the weapon from a special material.

Applying a coating takes **1 AP** and requires a free hand. A coated weapon holds **3 doses** â each hit against a living target expends one dose. Coatings can also be applied to arrowheads or bolts individually (1 dose each).

| Coating | Delivery | Effect on Hit | Cost per Dose |
| :--- | :--- | :--- | :--- |
| Silver paste | Injury | Counts as a silver weapon: +1 damage die vs. [Vampire](../Skills/Lineages/Vampire%20Lineage.md) and [Werewolf](../Skills/Lineages/Werewolf%20Lineage.md) lineages; suppresses healing until cleansed (see each lineage entry) | 25 gp |
| Wolfsbane | Injury | Delivers [Wolfsbane](../Conditions%20and%20Hazards/Poisons.md) poison to the target on a hit that deals damage | 25 gp |

1. Silver paste produces the same mechanical effects as a silver weapon against affected lineages. It does not make the weapon physically silver â a coated iron blade is still iron and still triggers iron/steel effects against Faerie and Fae-Touched.
2. Wolfsbane coating uses the Injury delivery type. The full wolfsbane poison entry â including the werewolf-specific healing suppression â applies on a successful hit that deals damage.
3. Wiping a coated weapon clean requires 1 AP and a cloth or rag. A coating on an arrowhead or bolt in the quiver does not expire â only fired shots expend doses.
